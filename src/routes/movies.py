...
@router.post("/movies/", status_code=status.HTTP_201_CREATED, response_model=MovieModel)
async def create_movie(
    ...
    try:
        country = db.scalar(select(CountryModel).where(CountryModel.code == movie_data.country))
        if not country:
